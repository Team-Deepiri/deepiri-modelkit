...
class AdvancedCacheManager:
    ...
    def get(
        self, key: str, namespace: str = "rag", update_access: bool = True
    ) -> Optional[Any]:
    ...
    def invalidate_by_pattern(self, pattern: str, namespace: str = "rag") -> int:
        ...
        # Memory cache
        keys_to_delete = [
            k for k in self.memory_cache.keys() if self._match_pattern(k, pattern)
        ]
        for key in keys_to_delete:
            self.delete(key, namespace)
            count += 1
...
class EmbeddingCache:
    ...
    def set(self, text: str, embedding: Any, ttl: int = 86400):
        ...
        return self.cache_manager.set(
            key, embedding, namespace=self.namespace, ttl=ttl, tags=["embedding"]
        )
...
class QueryResultCache:
    ...
    def set(
        ...
        if query.industry:
            query_tags.append(
                f"industry:{query.industry.value if hasattr(query.industry, 'value') else query.industry}"
            )
        if query.doc_types:
        ...
        return self.cache_manager.set(
            key, serialized, namespace=self.namespace, ttl=ttl, tags=query_tags
        )
    ...
    def invalidate_by_industry(self, industry: str):
        ...
        return self.cache_manager.invalidate_by_tag(
            f"industry:{industry}", namespace=self.namespace
        )

    def invalidate_by_doc_type(self, doc_type: str):
        """Invalidate all queries for a document type"""
        return self.cache_manager.invalidate_by_tag(
            f"doctype:{doc_type}", namespace=self.namespace
        )
