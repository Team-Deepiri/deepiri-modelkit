...
class RegulationProcessor(DocumentProcessor):
    ...
    def _extract_sections(self, content: str) -> List[Dict[str, Any]]:
        ...
        if not sections:
            ...
            sections = [
                {"section": f"Chunk {i+1}", "content": chunk}
...
class HistoricalDataProcessor(DocumentProcessor):
    ...
    def process(self, raw_content: str, metadata: Dict[str, Any]) -> List[Document]:
        ...
        doc_type = (
            DocumentType(doc_type_str) if doc_type_str else DocumentType.WORK_ORDER
        )

        # Historical data is typically already structured
        # We may need minimal chunking
        chunks = (
            self.chunk_text(raw_content)
            if len(raw_content) > self.chunk_size
            else [raw_content]
        )
...
class KnowledgeBaseProcessor(DocumentProcessor):
    ...
    def _extract_qa_pairs(self, content: str) -> List[Dict[str, str]]:
        ...
        if matches:
            qa_pairs.extend(
                [{"question": q.strip(), "answer": a.strip()} for q, a in matches]
            )
    ...
    def _process_qa_pairs(
        self, qa_pairs: List[Dict[str, str]], metadata: Dict[str, Any]
    ) -> List[Document]:
